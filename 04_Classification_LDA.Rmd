...
and therefore $$\hat{p} = \frac{1}{1+e^{-X\beta}} = \frac{1}{1+e^{-3.482}} = 0.0297$$
```{r}
newdata = data.frame( BNucl=2, UShap=1, USize=2 )
...
```{r}
# Calculate the ROC information
myROC <- roc( wbca$Class,  wbca$phat )

# make a nice plot using ggplot2
ggroc( myROC ) 
```
...
It is a little challenging to read the graph to see what the Sensitivity is for a particular value of Specificity. The in addition to displaying the ROC curve, the function `plot.roc()` also returns a list of the information it used to create the graph.

```{r}
foo <- plot.roc( myROC )
index <- which.min( abs( foo$sensitivities - 0.99 ) )
foo$specificities[index]
```

While you can obtain the estimated Specificity using this trick, the authors of `pROC` don't want you to do this because they would prefer you to also estimate the confidence intervals for the quantity using bootstrapping.
```{r, cache=TRUE}
...
$$P( y^* = i | x^* ) = \frac{ f_i(x^*) }{ \sum_j f_j(x^*) }$$
...
that are virginica, 22 are correctly identified as virginica while 3 are misclassified as versicolor. So out of 75 test cases, we correctly classified 57, and incorrectly classified 18.
...
test %>%
  summarise( misclassification_rate = mean( Species != yhat ) )
```
This has improved our accuracy as 4 setosa observations that were previously misclassified are now correctly classified.
...
5. ISL Chapter 4, problem 11: In this problem, you will develop a model to predict whether a given car gets high or low gas mileage based on the Auto data set. 
...
6. ISL Chapter 4, problem 13: Using the Boston data set, fit classification models in order to predict whether a given suburb has a crime rate above or below the median. Explore logistic regression, LDA, and KNN models using various subsets of the predictors. Describe your findings.