...
# clean up the data
# benign is considered the "success"
wbca <- wbca %>% 
  mutate(Class = ifelse(Class==0, 'malignant', 'benign')) %>%
  dplyr::select(Class, Adhes, BNucl, Epith, UShap, USize) 

# Fit the model
model <- glm( I(Class=='benign') ~ ., data=wbca, family='binomial' )
...
As usual we can calculate the summary statistics...

## Linear Discriminent Analysis

These classification techniques assume that we have a categorical (possibly unordered) response $y$ and a continuous covariate predictor $x$.  In many ways we can consider this the inverse problem of a 1-way anova.
