...
3. ISLR #6.9 In this exercise, we will predict the number of applications received using the other variables in the `College` data set available in the ISLR. 
    ...
    e) Fit a lasso model on the training set, with $\lambda$ chosen by cross-validation. Report the estimated error rate from the cross-validation as well as the observed test error obtained.  Also report the number of non-zero coefficient estimates. 
    f) Fit a lasso model on the training set, with $\lambda$ chosen by cross-validation. Report the estimated error rate from the cross-validation as well as the observed test error obtained.  Also report the number of non-zero coefficient estimates.  
