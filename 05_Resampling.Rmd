...
ggplot(ValidationSets_results, aes(x=factor(p), y=MSE)) + 
  geom_boxplot() +
...
save(LOOCV_results, file='Simulations/LinearModel_LOOCV.RData')
```
...
Total_Results <- rbind( 
  ValidationSets_results %>% 
    mutate(method=paste('VS_',p,sep='')) %>% 
    dplyr::select(MSE,method),
  LOOCV_results %>% 
    mutate(method='LOOCV') %>% 
    dplyr::select(MSE, method)  ) %>%
  filter(is.element(method, c('VS_0.5', 'VS_0.7', 'VS_0.8', 'LOOCV'))) 

ggplot(Total_Results, aes(x=method, y=MSE)) + 
  geom_boxplot() +
  geom_hline(yintercept = 1, color='red')
...
Total_Results <- rbind(
  Total_Results,
  KfoldCV_results %>% 
    mutate(method=paste(K,'-fold',sep='')) %>% 
    dplyr::select(MSE, method) ) 
